...
const EndpointDetailChart: React.FC<EndpointDetailChartProps> = ({ endpointName, intermediateData }) => {
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Tempos de Resposta para ${endpointName} (ms)`,
        color: '#FFFFFF',
        font: { size: 16 }
      }
    },
  ...
  const percentiles: (keyof NonNullable<IntermediateData['summaries'][string]>)[] = ['min', 'mean', 'p50', 'p75', 'p95', 'p99', 'max'];
  ...
  const data = {
    datasets: percentiles.map(p => ({
      label: `p${p.toString().replace('mean', 'médio')}`,
  ...
  return <div style={{ height: '350px' }}><Line options={options} data={data} /></div>;
};
