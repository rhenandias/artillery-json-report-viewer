...
const COLORS = [
  '#f43f5e', '#38bdf8', '#22c55e', '#f97316', '#a78bfa', '#6366f1',
  '#ec4899', '#14b8a6', '#facc15', '#d946ef', '#e11d48', '#8b5cf6'
];
...
const MetricsSidebar: React.FC<MetricsSidebarProps> = ({ chartConfigs, intermediateData }) => {
    ...
    const groupedMetrics = useMemo(() => {
        ...
        chartConfigs.forEach(config => {
            ...
            else if (config.title.includes('transaction')) category = 'Plugins › metrics-by-endpoint';
        ...
        <div className="bg-gray-800 p-4 rounded-lg shadow-lg">
            ...
            <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2">
                {Object.entries(groupedMetrics).map(([category, configs]) => (
                    <div key={category}>
                            <h4 className="font-bold text-sm text-blue-400 uppercase mb-2 tracking-wider">{category}</h4>
                            ...
                            {configs.map((config, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="text-sm text-gray-300 truncate">{config.title.replace(/.*\./, '')}</span>
                                        <Sparkline data={dataPoints} color={COLORS[index % COLORS.length]} />
                                    </div>
                                );
                            })}
                            </div>
