...
export const getMetricValue = (item: IntermediateData, key: string): number | null => {
  ...
  if (parts.length > 1) {
    ...
    if (item.summaries[summaryKey] && typeof item.summaries[summaryKey]?.[subKey] !== 'undefined') {
